// <auto-generated />
using Elo.DbHandler;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage;
using Microsoft.EntityFrameworkCore.Storage.Internal;
using System;

namespace Elo.DbHandler.Migrations
{
    [DbContext(typeof(EloDbContext))]
    [Migration("20180409195224_RatingChangeInPlayerRating")]
    partial class RatingChangeInPlayerRating
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "2.0.1-rtm-125")
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("Elo.Models.Game", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<DateTime>("Created")
                        .ValueGeneratedOnAddOrUpdate()
                        .HasDefaultValueSql("getutcdate()");

                    b.HasKey("Id");

                    b.ToTable("Games");
                });

            modelBuilder.Entity("Elo.Models.GameScore", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<DateTime>("Created")
                        .ValueGeneratedOnAddOrUpdate()
                        .HasDefaultValueSql("getutcdate()");

                    b.Property<int>("GameId");

                    b.Property<int>("PlayerId");

                    b.Property<double>("Score");

                    b.HasKey("Id");

                    b.HasIndex("GameId");

                    b.HasIndex("PlayerId");

                    b.ToTable("GameScores");
                });

            modelBuilder.Entity("Elo.Models.Player", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<DateTime>("Created")
                        .ValueGeneratedOnAddOrUpdate()
                        .HasDefaultValueSql("getutcdate()");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50);

                    b.HasKey("Id");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("Players");
                });

            modelBuilder.Entity("Elo.Models.PlayerRating", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<DateTime>("Created")
                        .ValueGeneratedOnAddOrUpdate()
                        .HasDefaultValueSql("getutcdate()");

                    b.Property<int>("CurrentStreak");

                    b.Property<int?>("GameId");

                    b.Property<int>("Losses");

                    b.Property<double>("Rating");

                    b.Property<double>("RatingChange");

                    b.Property<int>("Wins");

                    b.HasKey("Id");

                    b.HasIndex("GameId");

                    b.ToTable("PlayerRatings");
                });

            modelBuilder.Entity("Elo.Models.PlayerSeason", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<DateTime>("Created")
                        .ValueGeneratedOnAddOrUpdate()
                        .HasDefaultValueSql("getutcdate()");

                    b.Property<int>("CurrentPlayerRatingId");

                    b.Property<int>("PlayerId");

                    b.Property<int>("SeasonId");

                    b.HasKey("Id");

                    b.HasIndex("CurrentPlayerRatingId")
                        .IsUnique();

                    b.HasIndex("PlayerId");

                    b.HasIndex("SeasonId");

                    b.ToTable("PlayerSeasons");
                });

            modelBuilder.Entity("Elo.Models.PlayerSeasonRating", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<DateTime>("Created")
                        .ValueGeneratedOnAddOrUpdate()
                        .HasDefaultValueSql("getutcdate()");

                    b.Property<int>("PlayerRatingId");

                    b.Property<int>("PlayerSeasonId");

                    b.HasKey("Id");

                    b.HasIndex("PlayerRatingId")
                        .IsUnique();

                    b.HasIndex("PlayerSeasonId");

                    b.ToTable("PlayerSeasonRatings");
                });

            modelBuilder.Entity("Elo.Models.Season", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<DateTime>("Created")
                        .ValueGeneratedOnAddOrUpdate()
                        .HasDefaultValueSql("getutcdate()");

                    b.Property<DateTimeOffset>("EndDate");

                    b.Property<string>("Name")
                        .IsRequired();

                    b.Property<DateTimeOffset>("StartDate");

                    b.HasKey("Id");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("Seasons");
                });

            modelBuilder.Entity("Elo.Models.GameScore", b =>
                {
                    b.HasOne("Elo.Models.Game", "Game")
                        .WithMany("Scores")
                        .HasForeignKey("GameId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Elo.Models.Player", "Player")
                        .WithMany("GameScores")
                        .HasForeignKey("PlayerId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Elo.Models.PlayerRating", b =>
                {
                    b.HasOne("Elo.Models.Game", "Game")
                        .WithMany()
                        .HasForeignKey("GameId");
                });

            modelBuilder.Entity("Elo.Models.PlayerSeason", b =>
                {
                    b.HasOne("Elo.Models.PlayerRating", "CurrentPlayerRating")
                        .WithOne()
                        .HasForeignKey("Elo.Models.PlayerSeason", "CurrentPlayerRatingId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Elo.Models.Player", "Player")
                        .WithMany("Seasons")
                        .HasForeignKey("PlayerId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Elo.Models.Season", "Season")
                        .WithMany()
                        .HasForeignKey("SeasonId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Elo.Models.PlayerSeasonRating", b =>
                {
                    b.HasOne("Elo.Models.PlayerRating", "PlayerRating")
                        .WithOne()
                        .HasForeignKey("Elo.Models.PlayerSeasonRating", "PlayerRatingId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Elo.Models.PlayerSeason", "PlayerSeason")
                        .WithMany("Ratings")
                        .HasForeignKey("PlayerSeasonId")
                        .OnDelete(DeleteBehavior.Cascade);
                });
#pragma warning restore 612, 618
        }
    }
}
